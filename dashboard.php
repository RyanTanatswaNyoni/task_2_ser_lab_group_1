<?php
date_default_timezone_set('Africa/Lusaka'); // Sets PHP to your local time
...
if (isset($_GET["complete"])) {
    $id = $_GET["complete"];
    // Update both the status AND the updated_at time
...
function timeAgo($datetime) {
    date_default_timezone_set('Africa/Lusaka'); // Ensure consistency inside the function
    $created_at = new DateTime($datetime);
    $now = new DateTime();
    $interval = $created_at->diff($now);

    // If the time is in the future (due to slight clock desync), return just now
    if ($interval->invert) {
        return "just now";
    }
